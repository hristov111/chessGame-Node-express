...
const updateProfileFunc = async (req, res) => {
  try {
    const { id, username, firstname, lastname, email, biography } = req.body;
    console.log("Made passed!: " + id);
...
const updateGameSearchState = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ msg: "Game search state updated successfully" });
  } catch (error) {
...
const getUserByUsernameFunc = async (req, res) => {
  try {
    ...
    if (!user)
      return res
        .status(404)
        .json({ msg: `There isn't a person with this username: ${username}` });
...
const createUserFunc = async (req, res) => {
  try {
    ...
    res
      .status(409)
      .json({ msg: `The username is already in use: ${username}` });
  } catch (error) {
    res.status(500).json({ msg: error });
  }
};
...
const getGameSearchingUsers = async (req, res) => {
  try {
    ...
    if (!users)
      return res.status(404).json({ msg: "No active game searchers" });
...
const restoreGuestFunc = async (req, res) => {
  ...
  return res
    .status(200)
    .json({ msg: "Session restored", use: req.session.user });
};
